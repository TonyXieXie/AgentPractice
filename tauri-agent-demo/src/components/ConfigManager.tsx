...
export default function ConfigManager({ onClose, onConfigCreated, currentSessionId }: ConfigManagerProps) {
    ...
    const resolveModelImportSource = (data: unknown): unknown => {
        if (!isRecord(data)) return data;
        if (isRecord(data.models)) return data.models;
        if (isRecord(data.llm_configs)) return data.llm_configs;
        return data;
    };

    const extractConfigImportItems = (data: unknown): unknown[] => {
        const source = resolveModelImportSource(data);
        if (Array.isArray(source)) return source;
        if (isRecord(source)) {
            if (Array.isArray(source.configs)) return source.configs;
    ...
    const handleImportConfigs = async () => {
        ...
        if (!result) return;
        if (!(isAllowedKind(result.kind, MODEL_IMPORT_KINDS) || isAllowedKind(result.kind, ALL_IMPORT_KINDS))) {
    ...
    const extractGlobalImportPayload = (data: unknown): AppConfigUpdate | null => {
        if (!isRecord(data)) return null;
        const candidate = isRecord(data.global)
            ? data.global
    ...
    const handleImportGlobalConfig = async () => {
        ...
        if (!result) return;
        if (!(isAllowedKind(result.kind, GLOBAL_IMPORT_KINDS) || isAllowedKind(result.kind, ALL_IMPORT_KINDS))) {
    ...
    const extractAgentImportPayload = (data: unknown): AgentConfig | null => {
        if (!isRecord(data)) return null;
        const candidate = isRecord(data.agent)
            ? data.agent
    ...
    const handleImportAgentConfig = async () => {
        ...
        if (!result) return;
        if (!(isAllowedKind(result.kind, AGENT_IMPORT_KINDS) || isAllowedKind(result.kind, ALL_IMPORT_KINDS))) {
    ...
    const importModelConfigsFromItems = async (
        ...
        for (const payload of payloads) {
            ...
            try {
                ...
                if ('api_key' in debugPayload) {
                    debugPayload.api_key = debugPayload.api_key ? '[REDACTED]' : '';
                }
                console.error('[ConfigImport] Failed to import model config', {
                    name,
    ...
    const handleImportAllConfigs = async () => {
        ...
        const modelSource = resolveModelImportSource(data);
