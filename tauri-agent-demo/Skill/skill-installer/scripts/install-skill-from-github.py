#!/usr/bin/env python3
"""Install a skill from a GitHub repo path into the Agent Skill directory."""
...
def _agent_skill_root() -> str:
    env_root = os.environ.get("TAURI_AGENT_SKILL_DIR")
    if env_root:
        return env_root
...
def _tmp_root() -> str:
    base = os.path.join(tempfile.gettempdir(), "tauri-agent")
    os.makedirs(base, exist_ok=True)
    return base


def _request(url: str) -> bytes:
    return github_request(url, "tauri-agent-skill-install")
