...
class HttpRouteTests(unittest.TestCase):
    ...
    def test_session_busy_returns_queued_ticket(self) -> None:
        ToolRegistry.clear()
        ...
        try:
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                with TestClient(app) as client:
                    ...
                    detail = busy.json()
                    self.assertEqual(detail.get("status"), "queued")
                    self.assertEqual(detail.get("session_id"), session_id)
                    self.assertTrue(detail.get("ticket_id"))

                    snapshot_response = self._wait_for_snapshot(client, run_id)
                    self.assertEqual(snapshot_response.status_code, 200)

                    ticket_response = self._wait_for_ticket(
                        client,
                        detail["ticket_id"],
                    )
                    self.assertEqual(ticket_response.status_code, 200)
                    ticket_payload = ticket_response.json()
                    self.assertEqual(ticket_payload["status"], "started")
                    self.assertTrue(ticket_payload.get("run_id"))
        finally:
            ToolRegistry.clear()
    ...
    def _wait_for_ticket(self, client: TestClient, ticket_id: str):
        deadline = time.time() + 2.0
        last_response = None
        while time.time() < deadline:
            last_response = client.get(f"/run-tickets/{ticket_id}")
            if last_response.status_code == 200:
                status = last_response.json()["status"]
                if status in {"started", "rejected"}:
                    return last_response
            time.sleep(0.02)
        return last_response
