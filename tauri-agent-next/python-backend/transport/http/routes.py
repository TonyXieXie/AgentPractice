...
@router.get("/run-tickets/{ticket_id}", response_model=RunTicketResponse)
async def get_run_ticket(request: Request, ticket_id: str) -> RunTicketResponse:
    services = request.app.state.services
    ticket = await services.run_request_queue.get(ticket_id)
    if ticket is None:
        raise HTTPException(status_code=404, detail=f"run ticket not found: {ticket_id}")
